...
test("readPlatformImpersonationCookieSessionId devuelve la sesión firmada para el actor correcto", async () => {
  const secret = "0123456789abcdef0123456789abcdef"
  ...
  assert.equal(
    await readPlatformImpersonationCookieSessionId(cookieValue, "actor-1", secret),
    "support-session-1"
  )
})

test("readPlatformImpersonationCookieSessionId rechaza un actor distinto o cookie manipulada", async () => {
  const secret = "0123456789abcdef0123456789abcdef"
  const cookieValue = await buildPlatformImpersonationCookieValue(
    {
      actorUserId: "actor-1",
  ...
  assert.equal(await readPlatformImpersonationCookieSessionId(cookieValue, "actor-2", secret), null)
  assert.equal(await readPlatformImpersonationCookieSessionId("tampered", "actor-1", secret), null)
})
