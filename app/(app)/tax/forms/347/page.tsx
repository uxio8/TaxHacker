import { Model347DraftView } from "@/components/tax/forms/347/model-347-draft-view"
...
export const metadata = {
  title: "Modelo 347 | TaxHacker",
  description: "Gate anual para operaciones con terceros basado en calidad de contrapartes.",
}
...
export default async function Model347Page({
  ...
  try {
    ...
    const obligation = await getFiscalObligationByCodeAndPeriod(organizationId, "347", `${fiscalYear}-Y`)
    ...
    return (
      <main className="flex w-full max-w-7xl self-center flex-col gap-6 p-5">
        <Model347DraftView periodLabel={`${fiscalYear}-Y`} gate={gate} obligation={obligation} />
      </main>
    )
  } catch (error) {
