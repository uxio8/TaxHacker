      ...
      <div className="mb-3 pb-2 border-b border-gray-100">
        ...
        <p className="text-xs text-gray-500">
          {data.totalTransactions === 1
            ? t("dashboard.graph.oneTransaction")
            : t("dashboard.graph.manyTransactions", { count: data.totalTransactions })}
        </p>
      </div>
      ...
      <div className="mb-3 space-y-1">
        {data.income > 0 && (
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-green-600">{t("dashboard.stats.totalIncome")}:</span>
            <span className="text-sm font-bold text-green-600">{formatCurrency(data.income, defaultCurrency)}</span>
          </div>
        )}
        {data.expenses > 0 && (
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-red-600">{t("dashboard.stats.totalExpenses")}:</span>
      ...
      {incomeCategories.length > 0 && (
        <div className="mb-3">
          <h4 className="text-xs font-semibold text-green-600 mb-2 uppercase tracking-wide">
            {t("dashboard.graph.incomeByCategory")}
          </h4>
      ...
      {expenseCategories.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-red-600 mb-2 uppercase tracking-wide">
            {t("dashboard.graph.expensesByCategory")}
          </h4>
          <div className="space-y-1">
