...
async function runCommand(label, command, args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    process.stdout.write(`[critical] $ ${[command, ...args].join(" ")}\n`)
...
const playwrightSpecs = collectUniqueEntries((surface) => surface.playwrightSpecs)
...
if (playwrightSpecs.length > 0) {
  await runCommand("Playwright critical smokes", "npx", ["playwright", "test", ...playwrightSpecs])
}
